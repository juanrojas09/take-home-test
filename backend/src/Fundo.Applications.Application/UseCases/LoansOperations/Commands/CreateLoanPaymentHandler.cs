    ...
    ,IContextService contextService,ILogger<CreateLoanPaymentHandler> logger ):IRequestHandler<CreateLoanPaymentCommand,Response<LoanDto>>
{
    ...
    {
        var errorList=ValidateRequest(request.RequestDto);
        if (errorList.Any())
        {
        ...
        var loan=await loanQuerySqlDb.FirstOrDefaultAsync(x=>x.Id==request.RequestDto.LoanId,false,x=>x.Applicant);
        ...
        var responseDto = new LoanDto(
            ...
            (loan.CurrentBalance==0 ? LoanStatusesEnum.PAID.ToString() : loan.Status.ToString()) ?? string.Empty
            );
        return Response<LoanDto>.Success(responseDto,"Pago de prestamo registrado con exito");
        
    }
    
    private List<string> ValidateRequest(LoanPaymentRequestDto requestDto)
    {
        ...
        return  !validator.Validate(requestDto).IsValid 
            ? new List<string>()
            : validator.Validate(requestDto).Errors.Select(e => e.ErrorMessage).ToList();
    }
